class VoteService
  ...
  def cast_vote(vote)
    Vote.create!(user_id: user, voteable_id: votable_id, vote_type: vote, voteable_type: votable_type)
  end
  ...
  def find_value
    Vote.find_by( user_id: user, voteable_id: votable_id, voteable_type: votable_type)
  end
end
